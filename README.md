...
### プレビュー画像の配置ルール
以下のいずれかの条件を満たす画像（`.png`, `.jpg`, `.jpeg`）が自動的に認識されます。

1. **ファイル名で紐付け**
   - SWFファイルと同じ名前にする。
   - 例: `fonts_apricot.swf` → `fonts_apricot.png`
2. **フォルダ内から自動検出**
   - SWFと同じフォルダ内に、ファイル名にキーワード（`sample`、`preview`、`image`）を含む画像を置く。
   - 例: `sample.jpg`, `preview_all.png`, `myfont_sample.jpeg`

### 活用イメージ
- **配布されているフォント**: MOD紹介に貼られている画像をSWFと同じフォルダに `sample.jpg` としておいておけばいちいちMODページ確認不要。
- **自作の確認用**: ゲーム内でのスクリーンショットを「SWF名.png」として保存しておけば、ツール上でいつでもフォントの雰囲気を再確認できます。

> [!TIP]
> 複数のSWFを一つのフォルダにまとめている場合は、ルール1（ファイル名一致）を使うと、リストでフォントを切り替えるたびに個別のプレビューが表示されるのでおすすめです。